...
void JAdminActor::blocking_run(const WebSocketConnectionPtr &wsConnPtr, std::string &&message,
...
json JAdminActor::handleTextMessage(const WebSocketConnectionPtr &wsConnPtr, std::string &&message, json in)
{
    if (!in.is_array()) {
        return json::array();
    }
...
json JAdminActor::handleBinaryMessage(const WebSocketConnectionPtr &wsConnPtr, std::string &message)
{
    json event;
    try {
