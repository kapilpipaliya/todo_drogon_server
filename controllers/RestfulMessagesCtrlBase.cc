                                  ...
                                  std::function<void(const HttpResponsePtr &)> &&callback)
{
    ...
    auto callbackPtr =
        std::make_shared<std::function<void(const HttpResponsePtr &)>>(
            std::move(callback));
    mapper.findAll([callbackPtr, this](const std::vector<Messages> &v) {
            Json::Value ret;
            ret.resize(0);
            for (auto &obj : v)
            {
                                     ...
                                     std::function<void(const HttpResponsePtr &)> &&callback)
{
    ...
    try 
    {
        Messages object = 
            (_masquerading? 
             Messages(*jsonPtr, _masqueradingVector) : 
             Messages(*jsonPtr));
        auto dbClientPtr = getDbClient();
        auto callbackPtr =
            std::make_shared<std::function<void(const HttpResponsePtr &)>>(
        ...
        mapper.insert(
            object,
            [callbackPtr, this](Messages newObject){
                if(_masquerading)
                {
                    (*callbackPtr)(HttpResponse::newHttpJsonResponse(
