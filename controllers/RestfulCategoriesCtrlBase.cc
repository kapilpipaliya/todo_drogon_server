                                    ...
                                    std::function<void(const HttpResponsePtr &)> &&callback)
{
    ...
    auto callbackPtr =
        std::make_shared<std::function<void(const HttpResponsePtr &)>>(
            std::move(callback));
    mapper.findAll([callbackPtr, this](const std::vector<Categories> &v) {
            Json::Value ret;
            ret.resize(0);
            for (auto &obj : v)
            {
                                       ...
                                       std::function<void(const HttpResponsePtr &)> &&callback)
{
    ...
    try 
    {
        Categories object = 
            (_masquerading? 
             Categories(*jsonPtr, _masqueradingVector) : 
             Categories(*jsonPtr));
        auto dbClientPtr = getDbClient();
        auto callbackPtr =
            std::make_shared<std::function<void(const HttpResponsePtr &)>>(
        ...
        mapper.insert(
            object,
            [callbackPtr, this](Categories newObject){
                if(_masquerading)
                {
                    (*callbackPtr)(HttpResponse::newHttpJsonResponse(
