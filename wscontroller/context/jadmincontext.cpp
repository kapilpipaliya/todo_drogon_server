...
json JAdminContext::handleBinaryEvent(json event, int next, std::string &message)
{
...
json JAdminContext::adminLogin( json event, json args)
{
...
json JAdminContext::adminLogout( json event, json )
{
    deleteAdminSession();
...
json JAdminContext::userRegister( json event, json args)
{
...
json JAdminContext::userLogin( json event, json args)
{
    auto sql = "select e.id from entity.entity e left join entity.entity_user as u on u.entity_id = e.id where e.email = $1 and u.password = $2";
    try {
        auto clientPtr = drogon::app().getDbClient("sce");
...
json JAdminContext::userId( json event, json )
{
    long c =  user;
    if (c != 0) {
...
// ------------------
json JAdminContext::thumb_data( json event, json args)
{
    // send meta_data
    json batch;
    json ret;
...
json JAdminContext::save_setting_attachment(json event, std::string &message)
{
    auto session_id = admin;
