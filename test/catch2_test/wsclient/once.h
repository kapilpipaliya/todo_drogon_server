...
struct MakeSequnce {};
template <int N>
struct MakeSequnce<N, typename std::enable_if<N >= 0>::type>
    : MakeSequnceHelper<N> {};
}  // namespace Helper
...
struct AutoDisconnecter<
  ...
  void operator()(Args &&... args) const {
    ...
    if (--remainingCalls <= 0) QObject::disconnect(*connection);
    ...
    const int maxArgs = Helper::CompatibleArgCount<
        Func, typename Helper::FuncMeta<Signal>::ArgList>::value;
    static_assert(maxArgs != -1,
...
template <typename Func>
struct AutoDisconnecter<
    Func, typename std::enable_if<!Helper::FuncMeta<Func>::IsFunctor>::type> {
  Func func;
  std::shared_ptr<QMetaObject::Connection> connection;
  mutable int remainingCalls = 1;
...
QMetaObject::Connection connect(const Object *sender, Signal signal,
                                Functor &&functor) {
  AutoDisconnecter<Functor, Signal> disconnecter(
      std::forward<Functor>(functor));
  return *disconnecter.connection =
             QObject::connect(sender, signal, disconnecter);
}
...
QMetaObject::Connection connect(const Object *sender, Signal signal,
  ...
  AutoDisconnecter<Functor, Signal> disconnecter(
      std::forward<Functor>(functor));
  return *disconnecter.connection =
             QObject::connect(sender, signal, context, disconnecter, type);
}

template <typename Object, typename Signal, typename Functor>
QMetaObject::Connection connect(const Object *sender, Signal signal,
                                int callCount, Functor &&functor) {
...
template <typename Object, typename Signal, typename Functor>
QMetaObject::Connection connect(const Object *sender, Signal signal,
                                int callCount, const QObject *context,
                                Functor &&functor,
                                Qt::ConnectionType type = Qt::AutoConnection) {
  /* For convenice, allow user to call a function 0 times: */
  if (callCount <= 0) return {};
...
QMetaObject::Connection connect(
  ...
  SlotAutoDisconnecter<Receiver, Slot, Signal> disconnecter(
      const_cast<Receiver *>(receiver), std::forward<Slot>(slot));
...
template <typename Sender, typename Signal, typename Slot>
QMetaObject::Connection connect(
    const Sender *sender, Signal signal, int callCount,
    const typename Helper::FuncMeta<Slot>::Object *receiver, Slot slot,
    Qt::ConnectionType type = Qt::AutoConnection) {
  /* For convenice, allow user to call a function 0 times: */
  if (callCount <= 0) return {};
  ...
  using Receiver = typename Helper::FuncMeta<Slot>::Object;
  SlotAutoDisconnecter<Receiver, Slot, Signal> disconnecter(
      const_cast<Receiver *>(receiver), std::forward<Slot>(slot), callCount);
  /* Use receiver as context: */
  return *disconnecter.connection =
             QObject::connect(sender, signal, receiver, disconnecter, type);
}
