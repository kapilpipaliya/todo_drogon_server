...
void music::service::Catalog::clean_empty_albums() {
  ...
  while (albumid = sql::Dba::fetch_assoc($db_results)) {
      $id          = $albumid['id'];
      $sql         = "DELETE FROM album WHERE id = ?";
      $db_results  = sql::Dba::write($sql, array($id));
  }*/
}
...
bool music::service::Catalog::delet(long catalog_id, const std::string& type) {
  ...
  auto db_results = sql::Dba::write(sql1, catalog_id);
  ...
  auto db_results2 = sql::Dba::write(sql2, catalog_id);
  ...
  auto db_results3 = sql::Dba::write(sql3, catalog_id);
