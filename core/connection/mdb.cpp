...
int M_DB::serverVersion() { return PQserverVersion(conn); }
...
bool M_DB::executeSQLParams(std::string &statement, int nParams,
                            const char *const *paramValues, bool logsql) {
  if (!isConnOk()) return false;
  // statement = statement.trimmed();
  // if (logsql) logSQL(statement, kLogMsg_App);
  res = PQexecParams(conn, statement.c_str(), nParams, nullptr, paramValues,
                     nullptr, nullptr, 0);
...
bool M_DB::execPrepared(std::string &stmName, int nParams,
                        const char *const *paramValues) {
  if (!isConnOk()) return false;
  // if (logsql) logSQL(statement, kLogMsg_App);
  res = PQexecPrepared(conn, stmName.c_str(), nParams, paramValues, nullptr,
                       nullptr, 0);  // only check is it PGRES_COMMAND_OK
  return errorCheck(stmName);
}

int M_DB::nParams() { return PQnparams(res); }

Oid M_DB::paramType(int paramNumber) { return PQparamtype(res, paramNumber); }

bool M_DB::errorCheck(std::string &stmName) {
  if (PQresultStatus(res) == PGRES_TUPLES_OK ||
      PQresultStatus(res) == PGRES_SINGLE_TUPLE ||
      PQresultStatus(res) == PGRES_COMMAND_OK) {
    lastErrorMessage =
        PQerrorMessage(conn) + std::string("(") + stmName + std::string(")");
    printf("%s%s%s%s\n", "executePreparedSQL: ", stmName.c_str(), "->",
           PQerrorMessage(conn));
    fflush(stdout);
    return true;
  } else {
...
int M_DB::rows() { return PQntuples(res); }

int M_DB::columns() { return PQnfields(res); }

char *M_DB::columnName(int columnNumber) { return PQfname(res, columnNumber); }

int M_DB::columnNumber(const char *columnName) {
  return PQfnumber(res, columnName);
}

Oid M_DB::columnTable(int columnNumber) { return PQftable(res, columnNumber); }

int M_DB::tableColumn(int columnNumber) {
  return PQftablecol(res, columnNumber);
}

int M_DB::formatCode(int columnNumber) { return PQfformat(res, columnNumber); }

Oid M_DB::columnType(int columnNumber) { return PQftype(res, columnNumber); }
...
char *M_DB::columnDataType(int columnNumber) {
  ...
  paramValues[0] = pchar;
  PGresult *res = PQexecParams(conn, statement.c_str(), nParams, nullptr,
...
int M_DB::colModifier(int columnNumber) { return PQfmod(res, columnNumber); }

int M_DB::colSize(int columnNumber) { return PQfsize(res, columnNumber); }
...
void M_DB::clearResult() { PQclear(res); }
/*
...
gibberish for(int i=0; i<statement.size(); i++) { QChar ch = statement[i]; if(ch
< 32 && ch != '\n' && ch != '\r' && ch != '\t') { statement.truncate(i>0?i-1:0);
            statement.append(tr("... <string can not be logged, contains binary
data> ..."));
...
void M_DB::loadExtensionsFromSettings() {
  ...
  std::stringList list = Settings::getValue("extensions",
  "list").toStringList(); for(const std::string& ext : list)
  {
...
bool M_DB::strbool(int rowNumber, int columnNumber) {
  ...
  throw std::runtime_error(
      "invalid value in tao::pq::result_traits<bool> for input: " +
      std::string(value));
}
...
void M_DB::strtojson(int rowNumber, int columnNumber) {
  // if a query result column is of json or jsonb type, then PQgetvalue will
  // return a pointer to the json text as a string
}
// so to encrypt a password use  crypt('mypassword', gen_salt('bf',10))  or
// whatever value other than 10 isn't too slow for you (default is 6) then use
// WHERE storedpass = crypt(supplied_pass, storedpass)   to check whether an
