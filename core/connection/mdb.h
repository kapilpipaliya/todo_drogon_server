...
public:
    ...
    bool executeSQLParams(std::string &statement, int nParams, const char *const *paramValues, bool logsql = true);
    ...
    bool execPrepared(std::string &stmName, int nParams, const char * const *paramValues);
    int nParams(); // Returns the number of parameters of a prepared statement.
    Oid paramType(int paramNumber); // Returns the data type of the indicated statement parameter. Parameter numbers start at 0.
    ...
    Oid columnTable(int columnNumber); // Returns the OID of the table from which the given column was fetched.
    int tableColumn(int columnNumber); // Returns the column number (within its table) of the column making up the specified query result column.Query-result column numbers start at 0, but table columns have nonzero numbers. Zero is returned if the column number is out of range,
    int formatCode(int columnNumber); //Returns the format code indicating the format of the given column. Column numbers start at 0. Format code zero indicates textual data representation, while format code one indicates binary representation.
    Oid columnType(int columnNumber); //Returns the data type associated with the given column number. The integer returned is the internal OID number of the type. Column numbers start at 0. You can query the system table pg_type to obtain the names and properties of the various data types. The OIDs of the built-in data types are defined in the file src/include/catalog/pg_type_d.h in the source tree.
