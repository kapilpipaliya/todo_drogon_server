...
std::string CondFormat::filterToSqlCondition(const std::string& value, PG_TYPES column_type, const std::string& encoding)
{
    ...
    if (ok) {
        ...
        val.clear();
        val2.clear();
        if(value.substr(0, 2) == ">=" || value.substr(0, 2) == "<=" || value.substr(0, 2) == "<>") {
        ...
        } else if(value.substr(0, 1) == "/" && value.substr(value.size()-1, 1) == "/" && value.length() > 2) {
            val = value.substr(1, value.length() - 2);
            op = "REGEXP";
            numeric = false;
        } else if(value.substr(0, 1) == "{" && value.substr(value.size()-1, 1) == "}") {
            if(value.length() > 2) {
                val = "ANY('" + value + "')";
                op = "=";
                numeric = true;
