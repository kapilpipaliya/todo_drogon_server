...
class Song : public BaseServiceAbs {
 ...
 public:
  /**
   * _get_ext_info
   * This function gathers information from the song_ext_info table and adds it
  ...
  // public static function compare_media_information($media, $new_media,
  // $string_array, $skip_array) private static function
  // clean_string_field_value($value)
  /**
   * update
  ...
  /**
   * _update_item
   * This is a private function that should only be called from within the song
   * class. It takes a field, value song id and level. first and foremost it
   * checks the level against $GLOBALS['user'] to make sure they are allowed to
   * update this record it then updates it and sets $this->{$field} to the new
   * value
   * @param string $field
   * @param mixed $value
   * @param int $song_id
   ...
   * These are items that aren't used normally, and often large/informational
   * only
   * @param string $field
   * @param mixed $value
   * @param int $song_id
  ...
  /**
   * get_fields
   * This returns all of the 'data' fields for this object, we need to filter
   * out some that we don't want to present to a user, and add some that don't
   * exist directly on the object but are related
   * @return array
   */
   ...
   * @discussion    returns the path of the song file stripped of the catalog
   * path used for mpd playback
   ...
   * @return string
   */
  // public static function generic_play_url($object_type, $object_id,
  // $additional_params, $player=null, $local=false)
  /**
   * play_url
   ...
   * @return string
   */
  // public static function play_url($oid, $additional_params='', $player=null,
  // $local=false)
  /**
   * Get stream name.
   * @return string
   ...
   * @return array|boolean
   */
  // public static function get_transcode_settings_for_media($source, $target =
  // null, $player = null, $media_type = 'song', $options=array())
  /**
   ...
   * @return array|boolean
   */
  // public function get_transcode_settings($target = null, $player = null,
  // $options=array())
  /**
   * Get lyrics.
   * @return array
