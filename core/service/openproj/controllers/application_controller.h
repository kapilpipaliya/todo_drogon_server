...
class ApplicationController : public ActionController::Base {
  ...
  // and starts a session if needed
   void find_current_user() {
    ...
    } else if ( cookies[OpenProject::Configuration['autologin_cookie_name']] && Setting.autologin?) {
      // auto-login feature starts a new session
      user = User.try_to_autologin(cookies[OpenProject::Configuration['autologin_cookie_name']])
      session[:user_id] = user.id if ( user) {
      user
    } else if ( params[:format] == 'atom' && params[:key] && accept_key_auth_actions.include?(params[:action])) {
  ...
  // Authorize the user for the requested action
   void authorize(ctrl = params[:controller], action = params[:action], global = false) {
  ...
  // Authorize the user for the requested action outside a project
   void authorize_global(ctrl = params[:controller], action = params[:action], global = true) {
  ...
  // TODO: this method is right now only suited for controllers of objects that somehow have an association to Project
   void find_object_and_scope() {
    model_object = this->class._model_object.find(params[:id]) if ( params[:id].present?) {
  ...
  // doesn't use the layout for api requests
   ...
   void default_breadcrumb() {
    name = l('label_' + this->class.name.gsub('Controller', '').underscore.singularize + '_plural')
    if ( name =~ /translation missing/i) {
      name = l('label_' + this->class.name.gsub('Controller', '').underscore.singularize)
    }
    name
  }
  ...
  // They should be called at the very } of a class definition or file,
