...
//   void prune(user: [], project_id: nil) {
...
//  class << self {
//     void prune_project_related(user_ids, projects) {
...
//      watchers_by_watchable_class.each { |watchable_class_string, class_candidates|
...
//     void watchers_in_projects(projects, user_ids) {
//      watchable_classes = active_watchable_classes(user_ids)

//      neutral_scope = where(Arel::Nodes::Equality.new(1, 0))

//      watchable_classes.inject(neutral_scope) { |aggregate_scope, watched_class|
...
//     void destroy_watchers_if_permission_missing(watchable_class, class_candidates) {
...
//     void prune_by_users(watchers_by_users, watchable_class) {
//      watchers_by_users.each { |user, watchers|
...
//     void prune_by_projects(watchers_by_projects, watchable_class) {
//      watchers_by_projects.each { |project, watchers|
...
//     void watchers_of_watchable(watchable, projects, user_ids) {
//      // By using
...
//     void active_watchable_classes(user_ids) {
