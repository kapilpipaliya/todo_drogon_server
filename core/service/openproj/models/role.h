...
namespace openproj {
namespace models {
class Role : public openproj::models::ActiveRecord::Base {
 public:
  void save() {
    auto clientPtr = drogon::app().getDbClient("sce");
    drogon::orm::Mapper<drogon_model::openproject6::Roles> mapper_roles(
        clientPtr);
    drogon_model::openproject6::Roles* roles = this;
    mapper_roles.insert(*roles);
  }
 ...
 public:
  ...
  // Built-in roles
  enum AAA { NON_BUILTIN = 0, BUILTIN_NON_MEMBER = 1, BUILTIN_ANONYMOUS = 2 };
  ...
  //      }
  //      if ( non_member_role.new_record?) { raise "Unable to create the
  //      non-member role." ;}
  //    }
  //    non_member_role
  //  }
  ...
  //      }
  //      if ( anonymous_role.new_record?) { raise "Unable to create the
  //      anonymous role." ;}
  //    }
  //    anonymous_role
  //  }
 ...
 private:
  ...
  //    this->allowed_permissions ||= permissions +
  //    OpenProject::AccessControl.public_permissions.map(&:name)
  //  }
