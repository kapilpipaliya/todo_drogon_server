#ifndef OPENPROJ_MODELS_ENUMERATION_H
#define OPENPROJ_MODELS_ENUMERATION_H
...
class Enumeration : public openproj::models::ActiveRecord::Base, public drogon_model::openproject6::Enumerations {
  public:
    ...
    inline void option_name() {
    //    nil
      };

    inline bool became_default_value() {
    //    is_default? && is_default_changed?
      };

    inline void unmark_old_default_value() {
    //    Enumeration.where(type: type).update_all(is_default: false)
      };

    // Overloaded on concrete classes
    inline int objects_count() {
        return 0;
      };
