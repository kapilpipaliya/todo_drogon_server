...
class BasicDataSeederFinal : public seeder::BasicDataSeeder {
 ...
 public:
  ...
  std::shared_ptr<Seeder> document_category_seed = std::make_shared<seeder::StandardSeeder::BasicData::DocumentCategorySeeder>();
  std::vector<std::shared_ptr<seeder::Seeder>>    data_seeder_classes() {
    ...
    data.push_back(std::move(document_category_seed));
    return data;
  }
};
}  // namespace StandardSeeder
