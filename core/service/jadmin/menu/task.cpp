...
namespace jadmin {
Task::Task(std::shared_ptr<websocket::JAdminContext> context_) : context(std::move(context_)) {
  query = sql::Query(sql::ObjectIdentifier("menu", "task", "m"));
  setupTable();
}

nlohmann::json Task::handleEvent(nlohmann::json event, unsigned long next,
                                 nlohmann::json args) {
