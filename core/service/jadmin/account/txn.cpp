...
void save_txn_order_item(json &args,
  ...
  std::string strSqlPostCategorySimpleFind =
      "SELECT id, txn_id, post_id, pcs, purity_id, tone_id, clarity_id, price, "
      "instruction FROM order1.order_item WHERE id = $1";
  std::string strSqlPostCategoryDel =
      "DELETE FROM order1.order_item WHERE id = $1";
  std::string strSqlPostCategoryInsert =
      "INSERT INTO order1.order_item (txn_id, post_id, pcs, purity_id, "
      "tone_id, clarity_id, price, instruction) VALUES ($1, $2, $3, $4, $5, "
      "$6, $7, $8) RETURNING id;";
  std::string strSqlPostCategoryUpdateAtt =
      "UPDATE order1.order_item SET (txn_id, post_id, pcs, purity_id, tone_id, "
      "clarity_id, price, instruction) = ROW($2, $3, $4, $5, $6, $7, $8, $9) "
      "WHERE id = $1;";
  ...
  for (auto i : args[0]["o_i_order_item"]) {
    if (!i[1].is_null()) {  // to pass null row
      newItems.push_back({i[0].get<long>(), i[1].get<long>(), i[2].get<int>(),
                          i[3].get<long>(), i[4].get<long>(), i[5].get<long>(),
                          i[6].get<float>(), i[7].get<std::string>()});
    }
  }
