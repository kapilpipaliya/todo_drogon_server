...
void save_Entity_Address(json &args,
    ...
    for (auto i : args[0]["ea_entity_address"]) {    // ea.id, ea.address_type_id, ea.line1, ea.line2, ea.line3, ea.city, ea.state, ea.country, ea.zipcode, ea.phone
        if (!i[1].is_null())
            inVector.push_back(
                {i[0].get<long>(), i[1].get<long>(), i[2].get<std::string>(), i[3].get<std::string>(), i[4].get<std::string>(), i[5].get<std::string>(),
    ...
    for (const auto& r : inVector) {
        ...
        if (y.empty()) {
            ...
            //if(y[0][0].as<long>() != r.addess_type_id || y[0][2].as<int>() != r.pcs || y[0][3].as<double>() != r.weight || y[0][4].as<double>() != r.price || y[0][5].as<bool>() != r.ismain) {
            Dba::writeInTrans(transPtr, strSqlPostCategoryUpdateAtt, r.id, r.addess_type_id, r.line1, r.line2, r.line3, r.city,
                            r.state, r.country, r.zipcode, r.phone, r.ismain);
            //}
        }
    }
...
json Entity::ins( json event, json args) {
    std::string strSql = "INSERT INTO entity.entity (  entity_type_id, no, sequence_id, slug, parent_id, legal_name, tax_no, first_name, middle_name, last_name, birth_date, start_date, end_date, salary, rate, active, pay_to_name, threshold, credit_limit, terms, discount, discount_terms, discount_account_id, ar_ap_account_id, cash_account_id, currency_id, price_group_id, email) "
                         "values($1, $2, NULLIF($3, 0::bigint), $4, NULLIF($5, 0::bigint), $6, $7, $8, $9, $10, $11, NULLIF($12, '')::date, NULLIF($13, '')::date, $14, $15, $16, $17, $18, $19, $20, $21, $22, NULLIF($23, 0::bigint), NULLIF($24, 0::bigint), NULLIF($25, 0::bigint), NULLIF($26, 0::bigint), NULLIF($27, 0::bigint), $28) returning id";
    auto clientPtr = drogon::app().getDbClient("sce");
    auto transPtr = clientPtr->newTransaction();
    try {
        auto x = Dba::writeInTrans(transPtr, strSql,
                                 args[0]["entity_type_id"].get<long>(),
                                 args[0]["no"].get<std::string>(),
                                 args[0]["sequence_id"].is_null() ? 0L : args[0]["sequence_id"].get<long>(),
                                 ...
                                 args[0]["end_date"].is_null() ? "" : args[0]["end_date"].get<std::string>(),
                                ...
                                args[0]["discount_account_id"].is_null() ? 0L : args[0]["discount_account_id"].get<long>(),
                                args[0]["ar_ap_account_id"].is_null() ? 0L : args[0]["ar_ap_account_id"].get<long>(),
                                args[0]["cash_account_id"].is_null() ? 0L : args[0]["cash_account_id"].get<long>(),
                                args[0]["currency_id"].is_null() ? 0L : args[0]["currency_id"].get<long>(),
                                args[0]["price_group_id"].is_null() ? 0L : args[0]["price_group_id"].get<long>(),
        ...
        Dba::writeInTrans(transPtr, strSqlUser, entity_id, args[0]["email"].get<std::string>(), args[0]["eu_password"].get<std::string>(),
...
json Entity::upd( json event, json args) {
    if (args[0]["id"].get<long>()) {
        std::string strSql = "update entity.entity set (  entity_type_id, no, sequence_id, slug, parent_id, legal_name, tax_no, first_name, middle_name, last_name, birth_date, start_date, end_date, salary, rate, active, pay_to_name, threshold, credit_limit, terms, discount, discount_terms, discount_account_id, ar_ap_account_id, cash_account_id, currency_id, price_group_id,  email)"
                             " = ROW($2, $3, NULLIF($4, 0::bigint), $5, NULLIF($6, 0::bigint), $7, $8, $9, $10, $11, $12, NULLIF($13, '')::date, NULLIF($14, '')::date, $15, $16, $17, $18, $19, $20, $21, $22, $23, NULLIF($24, 0::bigint), NULLIF($25, 0::bigint), NULLIF($26, 0::bigint), NULLIF($27, 0::bigint), NULLIF($28, 0::bigint), $29) where id=$1";
        auto clientPtr = drogon::app().getDbClient("sce");
        auto transPtr = clientPtr->newTransaction();
        try {
...
json Entity::del( json event, json args) {
    ...
    try {
        ...
        Dba::writeInTrans(transPtr, dele_("setting.notification", "where from_entity_id = $1"), entity_id);
        Dba::writeInTrans(transPtr, dele_("setting.notification", "where to_entity_id = $1"), entity_id);

        Dba::writeInTrans(transPtr, dele_("entity.entity_bank_account", "where entity_id = $1"), entity_id);
        Dba::writeInTrans(transPtr, dele_("entity.entity_contact", "where entity_id = $1"), entity_id);
        Dba::writeInTrans(transPtr, dele_("entity.entity_file", "where entity_id = $1"), entity_id);
        Dba::writeInTrans(transPtr, dele_("entity.entity_image", "where entity_id = $1"), entity_id);
        Dba::writeInTrans(transPtr, dele_("entity.entity_note", "where entity_id = $1"), entity_id);
        Dba::writeInTrans(transPtr, dele_("entity.entity_address", "where entity_id = $1"), entity_id);
        //Dba::writeInTrans(transPtr, "delete from user1.session where value->>'value' = $1;", entity_id);
        Dba::writeInTrans(transPtr, "delete from user1.session where entity_id = $1;", entity_id);
        Dba::writeInTrans(transPtr, dele_("entity.entity_user", "where entity_id = $1"),
        ...
        Dba::writeInTrans(transPtr, dele_("entity.entity", "where id = $1"), entity_id);
