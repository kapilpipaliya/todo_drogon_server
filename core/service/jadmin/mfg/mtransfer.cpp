...
namespace jadmin {
MTransfer::MTransfer(std::shared_ptr<websocket::JAdminContext> context_) : context(std::move(context_)) {
  query = sql::Query(sql::ObjectIdentifier("mfg", "m_transfer", "m"));
  setupTable();
}

nlohmann::json MTransfer::handleEvent(nlohmann::json event, unsigned long next,
                                      nlohmann::json args) {
