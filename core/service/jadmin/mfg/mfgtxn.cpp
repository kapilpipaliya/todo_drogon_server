...
namespace jadmin {
MfgTxn::MfgTxn(std::shared_ptr<websocket::JAdminContext> context_) : context(std::move(context_)) {
  query = sql::Query(sql::ObjectIdentifier("mfg", "mfg_txn", "m"));
  setupTable();
}

nlohmann::json MfgTxn::handleEvent(nlohmann::json event, unsigned long next,
                                   nlohmann::json args) {
...
nlohmann::json MfgTxn::ins(nlohmann::json event, nlohmann::json args) {
  return query.insBase(event, args, "name", "$1",
                       args[0]["name"].get<std::string>());
}

nlohmann::json MfgTxn::upd(nlohmann::json event, nlohmann::json args) {
