...
void Purity::setupTable() {
  ...
  auto metal_purity = sql::ObjectIdentifier("material", "purity_metal", "mp");
  ...
  query.setJoins({
      ...
      sql::Join("left", u2, "p.update_user_id = u2.id"),
  });
  query.setGroupBy({
      sql::GroupByColumn("p", "id"),
      sql::GroupByColumn("m", "id"),
...
nlohmann::json Purity::ins(nlohmann::json event, nlohmann::json args) {
  auto metal_purity_table = sql::ObjectIdentifier("material", "purity", "p");
  auto purity_metal_table =
      sql::ObjectIdentifier("material", "purity_metal", "mp");
