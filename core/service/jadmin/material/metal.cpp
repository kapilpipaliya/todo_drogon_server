...
namespace jewel {
namespace service {
Metal::Metal(std::shared_ptr<websocket::jewel::JAdminContext> context_)
...
nlohmann::json Metal::ins(nlohmann::json event, nlohmann::json args) {
  ...
  boost::replace_all(strSql, "%1", metal_table.schema());
  boost::replace_all(strSql, "%2", metal_table.name());
