...
namespace jadmin {
CSColor::CSColor(std::shared_ptr<websocket::JAdminContext> context_) : context(std::move(context_)) {
  query = sql::Query(sql::ObjectIdentifier("material", "cs_color", "gt"));
  setupTable();
}

nlohmann::json CSColor::handleEvent(nlohmann::json event, unsigned long next,
                                    nlohmann::json args) {
...
nlohmann::json CSColor::ins(nlohmann::json event, nlohmann::json args) {
  return query.insBase(event, args, "slug, name", "$1, $2",
                       args[0]["slug"].get<std::string>(),
                       args[0]["name"].get<std::string>());
}

nlohmann::json CSColor::upd(nlohmann::json event, nlohmann::json args) {
