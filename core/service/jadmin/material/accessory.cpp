...
namespace jadmin {
Accessory::Accessory(std::shared_ptr<websocket::JAdminContext> context_) : context(std::move(context_)) {
  query = sql::Query(sql::ObjectIdentifier("material", "accessory", "a"));
  setupTable();
}
...
void Accessory::setupTable() {
  ...
  query.setSelectedColumns({
      ...
      //        sql::SelectedColumn({"Material", "material_id", "", "a",
