...
namespace jadmin {
SettingType::SettingType(std::shared_ptr<websocket::JAdminContext> context_)
    : context(std::move(context_)) {
  query = sql::Query(sql::ObjectIdentifier("product", "setting_type", "t"));
  setupTable();
}
...
void SettingType::setupTable() {
  ...
  query.setSelectedColumns({
      ...
      //        sql::SelectedColumn({"Code", "slug", "", "t", PG_TYPES::TEXT,
      //        true}),
