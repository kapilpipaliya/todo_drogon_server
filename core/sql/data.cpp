...
// Note that these aren't all possible BOMs. But they are probably the most
// common ones. The size is needed at least for the ones with character zero in
// them.
...
bool isTextOnly(QByteArray data, const QString& encoding, bool quickTest) {
  ...
  if (startsWithBom(data)) return true;

  // Truncate to the first few bytes for quick testing
  int testSize = quickTest ? std::min(512, data.size()) : data.size();

  // If the quick test has been requested and we have to truncate the string, we
  // have to use an approach where truncated multibyte characters are not
  // interpreted as invalid characters.
  if (quickTest && data.size() > testSize) {
    ...
    if ((encoding.isEmpty() || encoding.startsWith("ISO-8859")) &&
        data.contains('\0'))
      return false;

    QTextCodec::ConverterState state;
    QTextCodec* codec = encoding.isEmpty()
                            ? QTextCodec::codecForName("UTF-8")
                            : QTextCodec::codecForName(encoding.toUtf8());
...
bool startsWithBom(const QByteArray& data) {
  return data.startsWith(bom3) || data.startsWith(bom2a) ||
         data.startsWith(bom2b) || data.startsWith(bom4a) ||
         data.startsWith(bom4b);
}
