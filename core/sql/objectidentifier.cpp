...
static std::string duplicate_char(std::string str, char to_replace) {
  ...
  while ((pos = str.find(to_replace, pos)) != std::string::npos) {
    ...
    pos += 2;  // Advance by two characters in order to avoid escaping a
...
std::string escapeIdentifier(const std::string& id) {
  switch (customQuoting) {
    ...
    case SquareBrackets:
      // There aren't any escaping possibilities for square brackets inside the
      // identifier, so we rely on the user to not enter these characters when
      // this kind of quoting is selected.
      return '[' + id + ']';
    case DoubleQuotes:
    default:
      // This may produce a 'control reaches end of non-void function' warning
      // if the default branch is removed, even though we have covered all
      // possibilities in the switch statement.
      return '"' + duplicate_char(id, '"') + '"';
  }
}
...
bool ObjectIdentifier::fromSerialised(const std::string& serialised) {
  ...
  size_name =
      std::stoul(serialised.substr(pos_comma + 1, pos_colon - pos_comma));
