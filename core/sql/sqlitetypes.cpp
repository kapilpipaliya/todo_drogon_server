...
StringVector escapeIdentifier(StringVector ids) {
  std::transform(ids.begin(), ids.end(), ids.begin(),
                 [](const std::string& id) { return escapeIdentifier(id); });
  return ids;
}

std::string joinStringVector(const StringVector& vec,
                             const std::string& delim) {
  return std::accumulate(
      vec.begin(), vec.end(), std::string(),
      [delim](const std::string& so_far, const std::string& s) {
 ...
 * This is a stupid helper class which sets the current locale as used by the
 * C++ standard library to the C locale. Upon destruction it resets it to
 * whatever the previous locale was. This is used to work around a problem in
 * Antlr's string comparison which because it is case-independent relies on the
 * current locale. However, when parsind SQL statements we don't want the locale
 * to interfere here. Especially the Turkish locale is problematic here because
 * of the dotted I problem.
 */
class SetLocaleToC {
 public:
  SetLocaleToC()
      : oldLocale(std::setlocale(LC_CTYPE,
                                 nullptr))  // Query current locale and save it
  {
...
class CreateTableWalker {
 public:
  explicit CreateTableWalker(antlr::RefAST r) : m_root(r) {}
...
class CreateIndexWalker {
 public:
  explicit CreateIndexWalker(antlr::RefAST r) : m_root(r) {}
...
bool Object::operator==(const Object& rhs) const {
  ...
  if (m_fullyParsed !=
      rhs.m_fullyParsed)  // We check for the fully parsed flag to make sure not
                          // to lose anything in some corner cases
    return false;
...
std::string ForeignKeyClause::toString() const {
  ...
  if (m_override.size()) return m_override;
  ...
  if (m_constraint.size()) result += " " + m_constraint;

  return result;
}

void ForeignKeyClause::setFromString(const std::string& fk) { m_override = fk; }

std::string ForeignKeyClause::toSql(const StringVector& applyOn) const {
  std::string result;
  if (!m_name.empty()) result = "CONSTRAINT " + escapeIdentifier(m_name) + " ";
  result += "FOREIGN KEY(" + joinStringVector(escapeIdentifier(applyOn), ",") +
...
std::string PrimaryKeyConstraint::toSql(const StringVector& applyOn) const {
  ...
  if (!m_name.empty()) result = "CONSTRAINT " + escapeIdentifier(m_name) + " ";
  result +=
      "PRIMARY KEY(" + joinStringVector(escapeIdentifier(applyOn), ",") + ")";

  if (!m_conflictAction.empty()) result += " ON CONFLICT " + m_conflictAction;
...
bool Field::operator==(const Field& rhs) const {
  ...
  if (m_notnull != rhs.m_notnull) return false;
  if (m_check != rhs.m_check) return false;
  if (m_defaultvalue != rhs.m_defaultvalue) return false;
  if (m_autoincrement != rhs.m_autoincrement) return false;
  if (m_unique != rhs.m_unique) return false;
  if (m_collation != rhs.m_collation) return false;

  return true;
}

std::string Field::toString(const std::string& indent,
                            const std::string& sep) const {
  std::string str = indent + escapeIdentifier(m_name) + sep + m_type;
  if (m_notnull) str += " NOT NULL";
  if (!m_defaultvalue.empty()) str += " DEFAULT " + m_defaultvalue;
  if (!m_check.empty()) str += " CHECK(" + m_check + ")";
  if (m_autoincrement) str += " PRIMARY KEY AUTOINCREMENT";
  if (m_unique) str += " UNIQUE";
  if (!m_collation.empty()) str += " COLLATE " + m_collation;
  return str;
}
...
bool Field::isReal() const {
  std::regex is_real("^(real|double|double precision|float)$",
                     std::regex::icase);
  return std::regex_match(m_type, is_real);
}

bool Field::isNumeric() const {
  std::regex is_numeric("(^(decimal))|(^(numeric|boolean|date|datetime)$)",
                        std::regex::icase);
...
Table& Table::operator=(const Table& rhs) {
  ...
  // Clear the fields and the constraints first in order to avoid duplicates
  // and/or old data in the next step
  fields.clear();
  m_constraints.clear();

  // Make copies of the fields and the constraints. This is necessary in order
  // to avoid any unwanted changes to the application's main database schema
  // representation just by modifying a reference to the fields or constraints
  // and thinking it operates on a copy.
...
bool Table::operator==(const Table& rhs) const {
  ...
  if (m_withoutRowid != rhs.m_withoutRowid) return false;
  if (m_virtual != rhs.m_virtual) return false;
  if (fields != rhs.fields) return false;

  // We need to compare the constraint maps manually here. The reason is that
  // the values are pointers and the default implementation would compare the
  // pointers not the actual objects.
  if (m_constraints.size() != rhs.m_constraints.size()) return false;
  for (auto it = m_constraints.cbegin(); it != m_constraints.end(); ++it) {
    // For each element in this map we get the list of all elements with the
    // same key from the other map. Then we loop through that list and check if
    // we find an element of the same type which produces the same SQL
    // substring. We use this approach to avoid casting both objects to their
    // actual type, then dereferencing it etc.
    auto range = rhs.m_constraints.equal_range(it->first);
    bool found_something = false;
    for (auto jt = range.first; jt != range.second; ++jt) {
      if (it->second->type() == jt->second->type() &&
          it->second->toSql(it->first) == jt->second->toSql(jt->first)) {
...
StringVector Table::fieldList() const {
  ...
  for (const Field& f : fields) sl.push_back(f.toString());
...
StringVector Table::fieldNames() const {
  ...
  for (const Field& f : fields) sl.push_back(f.name());

  return sl;
}

StringVector Table::rowidColumns() const {
  // For WITHOUT ROWID tables this function returns the names of the primary key
  // column. For ordinary tables with a rowid column, it returns "_rowid_"
  if (m_withoutRowid)
    return primaryKey();
  else
    return {"_rowid_"};
...
bool Table::hasAutoIncrement() const {
  return std::any_of(fields.begin(), fields.end(),
                     [](const Field& f) { return f.autoIncrement(); });
}
...
TablePtr Table::parseSQL(const std::string& sSQL) {
  ...
  try {
    ...
    std::cerr << "Sqlite parse error: " << ex.toString() << "(" << sSQL << ")"
              << std::endl;
  } catch (...) {
...
std::string Table::sql(const std::string& schema, bool ifNotExists) const {
  // Special handling for virtual tables: just build an easy create statement
  // and copy the using part in there
  if (isVirtual())
    return "CREATE VIRTUAL TABLE " +
           ObjectIdentifier(schema, m_name).toString(true) + " USING " +
           m_virtual + ";";
  ...
  while (it != m_constraints.cend()) {
    ...
    if ((!autoincrement ||
         it->second->type() != Constraint::PrimaryKeyConstraintType)) {
      // Ignore all constraints without any fields, except for check constraints
      // which don't rely on a field vector
      if (!(it->first.empty() &&
            it->second->type() != Constraint::CheckConstraintType)) {
        sql += ",\n\t";
  ...
  // without rowid
  if (withoutRowidTable()) sql += " WITHOUT ROWID";

  return sql + ";";
}

void Table::addConstraint(const StringVector& vStrFields,
                          ConstraintPtr constraint) {
  m_constraints.insert({vStrFields, constraint});
}

void Table::setConstraint(const StringVector& vStrFields,
                          ConstraintPtr constraint) {
  // Delete any old constraints of this type for these fields
  removeConstraints(vStrFields, constraint->type());

  // Add the new constraint to the table, effectively overwriting all old
  // constraints for that fields/type combination
  addConstraint(vStrFields, constraint);
}

void Table::removeConstraints(const StringVector& vStrFields,
...
ConstraintPtr Table::constraint(const StringVector& vStrFields,
                                Constraint::ConstraintTypes type) const {
...
std::vector<ConstraintPtr> Table::constraints(
    const StringVector& vStrFields, Constraint::ConstraintTypes type) const {
  ...
  std::transform(
      begin, end, std::back_inserter(clist),
      [](std::pair<StringVector, ConstraintPtr> elem) { return elem.second; });
  ...
  if (type == Constraint::NoType) {
    ...
    for (const ConstraintPtr& ptr : clist) {
      if (ptr->type() == type) clist_typed.push_back(ptr);
    }
    return clist_typed;
  }
}
...
StringVector& Table::primaryKeyRef() {
  return const_cast<StringVector&>(
      static_cast<const Table*>(this)->primaryKey());
}
...
void Table::renameKeyInAllConstraints(const std::string& key,
                                      const std::string& to) {
...
std::string identifier(antlr::RefAST ident) {
  ...
  if (ident->getType() == sqlite3TokenTypes::QUOTEDID ||
    ...
    // Replace all remaining occurences of two succeeding quote characters and
    // replace them by a single instance. This is done because two quotes can be
    // used as a means of escaping the quote character, thus only the visual
    // representation has its two quotes, the actual name contains only one.
    sident = std::regex_replace(
        sident, std::regex("\\" + quoteChar + "\\" + quoteChar), quoteChar);
  }
...
std::string textAST(antlr::RefAST t) {
  // When this is called for a KEYWORDASTABLENAME token, we must take the
  // child's content to get the actual value instead of 'KEYWORDASTABLENAME' as
  // a string. The same applies for  KEYWORDASCOLUMNNAME tokens.
  if (t != antlr::nullAST &&
      (t->getType() == sqlite3TokenTypes::KEYWORDASTABLENAME ||
       t->getType() == sqlite3TokenTypes::KEYWORDASCOLUMNNAME))
...
TablePtr CreateTableWalker::table() {
  ...
  if (m_root)  // CREATE TABLE
  {
    ...
    // If the primary tree isn't filled, this isn't a normal CREATE TABLE
    // statement. Switch to the next alternative tree.
    if (s == nullptr) s = m_root->getNextSibling();
    ...
    while (s->getType() != Sqlite3Lexer::ID &&
      ...
      if (s->getType() == Sqlite3Lexer::VIRTUAL) is_virtual_table = true;
    ...
    // Special handling for virtual tables. If this is a virtual table, extract
    // the USING part and skip all the rest of this function because virtual
    // tables don't have column definitons
    if (is_virtual_table) {
    ...
    // loop columndefs
    while (column != antlr::nullAST &&
           column->getType() == sqlite3TokenTypes::COLUMNDEF) {
      ...
      if (s->getType() != sqlite3TokenTypes::WITHOUT) {
        ...
        switch (tc->getType()) {
          case sqlite3TokenTypes::PRIMARY: {
            ...
            tc =
                tc->getNextSibling()->getNextSibling();  // skip primary and key
            ...
            do {
              ...
              if (indexed_column != antlr::nullAST &&
                  indexed_column->getType() == sqlite3TokenTypes::COLLATE) {
                indexed_column = indexed_column->getNextSibling();  // COLLATE
                // TODO save collation name
                tab->setFullyParsed(false);
                indexed_column =
                    indexed_column->getNextSibling();  // collation name
              }

              if (indexed_column != antlr::nullAST &&
                  indexed_column->getType() ==
                      sqlite3TokenTypes::AUTOINCREMENT) {
              ...
              while (tc != antlr::nullAST &&
                     tc->getType() == sqlite3TokenTypes::COMMA) {
                tc = tc->getNextSibling();  // skip ident and comma
              }
            } while (tc != antlr::nullAST &&
                     tc->getType() != sqlite3TokenTypes::RPAREN);
          ...
          case sqlite3TokenTypes::FOREIGN: {
            ...
            if (tc != antlr::nullAST &&
                tc->getType() == sqlite3TokenTypes::LPAREN) {
              tc = tc->getNextSibling();  // LPAREN
          ...
          default: {
            std::cout << "unknown table constraint in " << tab->name()
                      << std::endl;
            tab->setFullyParsed(false);
          } break;
        }
        ...
        s = s->getNextSibling();  // COMMA or RPAREN
        if (s->getType() == sqlite3TokenTypes::COMMA ||
            s->getType() == sqlite3TokenTypes::RPAREN)
          s = s->getNextSibling();
      } else {
        // It is
...
void CreateTableWalker::parsecolumn(Table* table, antlr::RefAST c) {
  ...
  if (c != antlr::nullAST && c->getType() == sqlite3TokenTypes::TYPE_NAME) {
    ...
    while (t != antlr::nullAST) {
      ...
      if (t != antlr::nullAST) {
        ...
        if (nextType != sqlite3TokenTypes::LPAREN &&
            nextType != sqlite3TokenTypes::RPAREN &&
  ...
  while (c != antlr::nullAST) {
    ...
    switch (con->getType()) {
      case sqlite3TokenTypes::PRIMARY: {
        // If we have already allocated a primary key object, delete it first.
        // This should never happen in real world situations because SQLite only
        // supports one primary key constraint per field.
        ...
        if (con != antlr::nullAST &&
            (con->getType() == sqlite3TokenTypes::ASC ||
        ...
        if (con != antlr::nullAST &&
            con->getType() == sqlite3TokenTypes::AUTOINCREMENT)
          autoincrement = true;
      } break;
      case sqlite3TokenTypes::NOT: {
        // TODO Support constraint names here
        if (!constraint_name.empty()) table->setFullyParsed(false);
      ...
      case sqlite3TokenTypes::CHECK: {
        ...
        // If we have a constraint name, convert this constraint from a column
        // into a table constaint in order to save it with our data model
      ...
      case sqlite3TokenTypes::REFERENCES: {
        ...
        if (con != antlr::nullAST &&
            con->getType() == sqlite3TokenTypes::LPAREN) {
          con = con->getNextSibling();  // LPAREN

          StringVector fk_cols;
          while (con != antlr::nullAST &&
                 con->getType() != sqlite3TokenTypes::RPAREN) {
      ...
      default: {
        std::cout << "unknown column constraint in " << table->name() << "."
                  << colname << std::endl;
        table->setFullyParsed(false);
      } break;
    }
  ...
  if (primaryKey) {
    ...
    if (table->constraint(v, Constraint::PrimaryKeyConstraintType)) {
      ...
      // Delete useless primary key constraint. There already is a primary key
      // object for this table, we don't need another one.
      delete primaryKey;
    } else {
...
std::string CreateTableWalker::parseConflictClause(antlr::RefAST c) {
  ...
  if (c != antlr::nullAST && c->getType() == sqlite3TokenTypes::ON &&
      c->getNextSibling()->getType() == sqlite3TokenTypes::CONFLICT) {
...
std::string IndexedColumn::toString(const std::string& indent,
                                    const std::string& sep) const {
...
StringVector Index::columnSqlList() const {
  ...
  for (const IndexedColumn& c : fields) sl.push_back(c.toString());
...
std::string Index::sql(const std::string& schema, bool ifNotExists) const {
  ...
  sql += "\n)";
  if (!m_whereExpr.empty()) sql += " WHERE " + m_whereExpr;
...
IndexPtr CreateIndexWalker::index() {
  ...
  if (m_root)  // CREATE INDEX
  {
    ...
    while (s->getType() != Sqlite3Lexer::ID &&
      ...
      if (s->getType() == Sqlite3Lexer::UNIQUE) index->setUnique(true);
    ...
    // loop columndefs
    while (column != antlr::nullAST &&
           column->getType() == sqlite3TokenTypes::INDEXEDCOLUMN) {
...
void CreateIndexWalker::parsecolumn(Index* index, antlr::RefAST c) {
  ...
  // First count the number of nodes used for the name or the expression. We
  // reach the end of the name nodes list when we either get to the end of the
  // list, get to a COMMA or a RPAREN, or get to the COLLATE keyword or get to
  // the ASC/DESC keywords. Then see how many items there are: if it's one it's
  // a normal index column with only a column name. In this case get the
  // identifier. If it's more than one item it's an expression. In this case get
  // all the items as they are.
  int number_of_name_items = 0;
  antlr::RefAST n = c;
  while (n != antlr::nullAST && n->getType() != sqlite3TokenTypes::COLLATE &&
