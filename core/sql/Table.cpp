...
unsigned long Table::rowCount() const { return result.size(); }

unsigned long Table::columnCount() const { return result.columns(); }
...
void Table::clear() {}
const char *Table::columnName(unsigned long columnNumber) const {
  if (result.empty()) {
    return m_query.selectedColumns().at(columnNumber).original_column.c_str();
  }
  return result.columnName(columnNumber);
}
bool Table::isEditableColumn(unsigned long column) const {
  if (column >= columnCount()) {
    return false;
  }
  return m_query.selectedColumns().at(column).isEditable;
}
std::string Table::getHeaderName(const unsigned long column) const {
...
void Table::sort(const std::vector<sqlb::SortedColumn> &columns) {
  ...
  if (m_query.orderBy() == columns) return;
...
void Table::updateFilterBase(json filters) {
  ...
  for (unsigned int i = 0; i < filters.size(); i++) {
    ...
    std::string whereClause = CondFormat::filterToSqlCondition(
        v, m_query.selectedColumns().at(i).column_type, "");
...
size_t Table::filterCount() const { return m_query.where().size(); }
