...
#include <QObject>
#include <QStringList>
// Slowly migrate this file to mdb.h
struct sqlite3;
...
class DBBrowserDB : public QObject {
 ...
 private:
  ...
  struct DatabaseReleaser {
    explicit DatabaseReleaser(DBBrowserDB* pParent_ = nullptr)
        : pParent(pParent_) {}

    DBBrowserDB* pParent;

    void operator()(sqlite3* db) const {
      if (!db || !pParent) return;
 ...
 public:
  ...
  // This returns the SQLite version as well as the SQLCipher if DB4S is
  ...
  **/
  ...
  bool dump(const QString& filename, const QStringList& tablesToDump,
            bool insertColNames, bool insertNew, bool exportSchema,
            bool exportData, bool keepOldSchema);

  enum ChoiceOnUse { Ask, Wait, CancelOther };
 ...
 private:
   ...
   * @param schemaName The name of the database schema in which to find the
   * table
   * @param pk_value This optional parameter can be used to manually set a
   * specific value for the primary key column
 ...
 public:
  ...
  /**
   * @brief This type maps from old column names to new column names. Given the
   * old and the new table definition, this suffices to track fields between the
   * two. USE CASES: 1) Don't specify a column at all or specify equal column
   * names: Keep its name as-is. 2) Specify different column names: Rename the
   * field. 3) Map from an existing column name to a Null string: Delete the
   * column. 4) Map from a Null column name to a new column name: Add the
   * column.
   */
  using AlterTableTrackColumns = QMap<QString, QString>;

  /**
   * @brief alterTable Can be used to rename, modify or drop existing columns of
   * a given table
   * @param tablename Specifies the schema and name of the table to edit
   * @param new_table Specifies the new table schema. This is exactly how the
   * new table is going to look like.
   * @param track_columns Maps old column names to new column names. This is
   * used to copy the data from the old table to the new one.
   * @param newSchema Set this to a non-empty string to move the table to a new
   * schema
 ...
 private:
  std::vector<std::pair<std::string, std::string> > queryColumnInformation(
 ...
 public:
  ...
  // This function generates the name for a temporary table. It guarantees that
  // there is no table with this name yet
 ...
 private:
  ...
  void collationNeeded(void* pData, sqlite3* db, int eTextRep,
                       const char* sCollationName);
  void errorLogCallback(void* user_data, int error_code, const char* message);

  bool tryEncryptionSettings(const QString& filename, bool* encrypted,
  ...
  class NoStructureUpdateChecks {
   public:
    explicit NoStructureUpdateChecks(DBBrowserDB& db) : m_db(db) {
      m_db.dontCheckForStructureUpdates = true;
    }
