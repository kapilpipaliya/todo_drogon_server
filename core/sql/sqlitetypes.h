...
using ConstraintMap =
    std::unordered_multimap<StringVector, ConstraintPtr, StringVectorHash>;
using FieldInfoList = std::vector<FieldInfo>;

struct FieldInfo {
  FieldInfo(const std::string& name_, const std::string& type_,
...
class Object {
 public:
  enum Types { Table, Index, View, Trigger };

  explicit Object(const std::string& name)
      : m_name(name), m_fullyParsed(false) {}
  virtual ~Object() {}
  ...
  void setOriginalSql(const std::string& original_sql) {
    m_originalSql = original_sql;
  }
   ...
   * @param ifNotExists If set to true the "IF NOT EXISTS" qualifier will be
   * added to the create statement
   * @return A std::string with the CREATE statement.
   */
  virtual std::string sql(const std::string& schema = std::string("main"),
...
class Constraint {
 public:
  ...
  explicit Constraint(const std::string& name = std::string()) : m_name(name) {}
  virtual ~Constraint() {}
...
class ForeignKeyClause : public Constraint {
 public:
  ForeignKeyClause(const std::string& table = std::string(),
                   const StringVector& columns = {},
                   const std::string& constraint = std::string())
      : m_table(table), m_columns(columns), m_constraint(constraint) {}
...
class PrimaryKeyConstraint : public Constraint {
 public:
  ...
  void setConflictAction(const std::string& conflict) {
    m_conflictAction = conflict;
  }
...
class Field {
 public:
  Field() : m_notnull(false), m_autoincrement(false), m_unique(false) {}

  Field(const std::string& name, const std::string& type, bool notnull = false,
        const std::string& defaultvalue = "", const std::string& check = "",
        bool unique = false, const std::string& collation = "")
      : m_name(name),
        m_type(type),
        m_notnull(notnull),
        m_check(check),
...
class Table : public Object {
 public:
  explicit Table(const std::string& name)
      : Object(name), m_withoutRowid(false) {}
   ...
   */
  std::string sql(const std::string& schema = "main",
                  bool ifNotExists = false) const override;

  StringVector fieldNames() const;

  StringVector rowidColumns() const;
  void setWithoutRowidTable(bool without_rowid) {
    m_withoutRowid = without_rowid;
  }
  bool withoutRowidTable() const { return m_withoutRowid; }

  void setVirtualUsing(const std::string& virt_using) {
    m_virtual = virt_using;
  }
  ...
  void removeConstraints(
      const StringVector& vStrFields = StringVector(),
      Constraint::ConstraintTypes type =
          Constraint::NoType);  //! Only removes the first constraint, if any
  ConstraintPtr constraint(
      const StringVector& vStrFields = StringVector(),
      Constraint::ConstraintTypes type = Constraint::NoType)
      const;  //! Only returns the first constraint, if any
  std::vector<ConstraintPtr> constraints(
      const StringVector& vStrFields = StringVector(),
      Constraint::ConstraintTypes type = Constraint::NoType) const;
...
class IndexedColumn {
 public:
  IndexedColumn(const std::string& name, bool expr,
                const std::string& order = std::string())
      : m_name(name), m_isExpression(expr), m_order(order) {}
...
class Index : public Object {
 public:
   ...
   * @return The index object. The index object may be empty if the parsing
   * failed.
   */
...
class View : public Object {
 public:
  ...
  std::string sql(const std::string& /*schema*/ = "main",
                  bool /*ifNotExists*/ = false) const override { /* TODO */
...
/**
 * @brief findField Finds a field in the database object and returns an iterator
 * to it.
 * @param object
 * @param name
 * @return The iterator pointing to the field in the field container of the
 * object if the field was found. object.fields.end() if the field couldn't be
 * found.
 */
template <typename T>
typename T::field_iterator findField(T* object, const std::string& name) {
  return std::find_if(object->fields.begin(), object->fields.end(),
                      [&name](const typename T::field_type& f) {
...
template <typename T>
typename std::remove_reference<T>::type::field_iterator findField(
    std::shared_ptr<T> object, const std::string& name) {
...
template <typename T,
          typename = typename std::enable_if<is_shared_ptr<T>::value>::type>
bool removeField(T object, const std::string& name) {
